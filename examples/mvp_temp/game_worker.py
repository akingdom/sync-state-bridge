#!/usr/bin/env python3
"""
Worker – game logic with command queue processed in main loop.
Commands are received via IPC and placed into a queue.
"""
import asyncio
import json
import math
...
# ----------------------------------------------------------------------
# GameState – unchanged except for command processing
# ----------------------------------------------------------------------
class GameState:
    def __init__(self):
    ...
    def handle_move(self, pid: int, keys: Dict[str, bool]):
        if pid not in self.player_keys:
            self.handle_join(pid)
        self.player_keys[pid] = keys

    def handle_fire(self, pid: int):
        if pid not in self.player_keys:
            self.handle_join(pid)
            ...
            try:
                ...
                if frame_type == FRAME_COMMAND:
                    ...
                    try:
                        ...
                        continue
                    # Put command into queue for main loop processing
        ...
        while True:
            ...
            while not command_queue.empty():
                ...
                if action == "game:join":
                    pid = params.get("pid")
                    if pid is not None:
                        game.handle_join(pid)
                elif action == "game:move":
                    pid = params.get("pid")
                    keys = params.get("keys", {})
                    if pid is not None:
                        game.handle_move(pid, keys)
                elif action == "game:fire":
                    pid = params.get("pid")
                    if pid is not None:
                        game.handle_fire(pid)
                elif action == "game:reset":
                    game.handle_reset()
                elif action == "game:spawn":
                    count = params.get("count", 100)
                    game.handle_spawn(count)
                elif action == "game:clear":
                    game.handle_clear()
                else:
